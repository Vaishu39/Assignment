...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, Modal, TextInput, ScrollView, Button, Image } from 'react-native'; // Importing essential UI components from React Native
...
const Home = () => {
  ...
  const handleSave = async () => {
    try {
      const url = flowerId ? `${API_BASE_URL}/${flowerId}` : API_BASE_URL;
      ...
      console.log("Flower saved successfully:", response.data);
      getListFlowers();
      handleCloseModal();
    } catch (error) {
      console.error("Error saving flower:", error.response?.data || error.message);
    }
  };
        ...
        {list.map((item) => (
          <View style={styles.itemContainer} key={item._id}>
            <View style={styles.item}>
              ...
              {/* Show selected image for the specific flower */}
